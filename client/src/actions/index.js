...
/* define the ActionCreators, which will return an action (i.e. a type and a payload) */
...
// this method gets all data for an event based on an event id
export function refreshActiveGuest(guestId, authToken) {
  const url = `/api/guest/one/${guestId}`;
  const config = {
    headers: { Authorization: `bearer ${authToken}` },
  };
  const request = axios.get(url, config);
